...
class NeovimSessionManager {
    ...
    private data class Entry(
        ...
        val bufferId: NeovimObject.BufferId? = null,
    )

    private val byEditor = ConcurrentHashMap<Editor, Entry>()
    private val bufferIdToEditor = ConcurrentHashMap<NeovimObject.BufferId, Editor>()
    ...
    suspend fun getSession(bufferId: NeovimObject.BufferId): NeovimEditorSession {
        val editor = bufferIdToEditor[bufferId] ?: error("BufferId not found: $bufferId")
        return getSession(editor)
    }

    fun getEditor(bufferId: NeovimObject.BufferId): Editor {
